...
### Verifying claims the model cannot check
...
with GitHub waiting for a status report that never arrives, and blocks merge forever with no
timeout. The fix is an administrator editing branch protection, and once a required check has
been removed under that pressure it never comes back.
