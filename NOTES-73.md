# NOTES on issue #73: bounded retry for parse_failure / deadline_exceeded

Worktree: `~/Source/cite/retry-73`, branch `fix/73-bounded-retry`, from `origin/main` (540f422).
Append findings here as you go: this file is what remains if the process stops.

## Reading of the issue (what I chose and why)

The issue's four bullets are directions, not prescriptions. My weighing:

1. **Echo guard (doing it, but as relabel, not re-ask).** The issue says a wrong-path
   echo is retriable "without spending a model round-trip". I weighed two readings:
   ...
   reviewed, it reviewed the bytes we sent; the `path` field only labels the payload. A
   ...
   every finding in that response (covered by a test). This makes the old wrong-path re-ask
   (previously drawn from the run-global bucket) free, which matters because the measured
   CI failures were predominantly exactly this pattern (`"unknown"`, `""`, `"README.md"`).
2. **Per-file parse budget (doing it, fully separate).** Output-contract failures
   ...
   run-global bucket reintroduces the starvation that the issue measures (attempt 2 lost four
...
3. **Deadline retry (doing it, exactly one, fresh budget).** Today terminal by design.
   The rationale ("a timeout retry pays twice for the same wait") is real, but the
   measurements favour a retry: attempt 1 of PR #71 lost README.md to a derived-deadline
...
purpose: a setting invites raising it until a fast red becomes a slow red.
...
- [x] Gate surfacing (red → green): `TestErroredReasonNamesMechanismAndBudgetSpent`. The red run
      showed the old reason `"file errored during review: bad.go, slow.go"`; green now
      ...
      `TestErroredWithoutMechanismStillNamesPaths` pins the old reason format (no reason/re-asks
...
- [ ] Draft PR linking #73 (the operator decides whether to mark it ready or merge).
