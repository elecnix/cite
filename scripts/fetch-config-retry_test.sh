#!/usr/bin/env bash
# Retry tests for scripts/fetch-config.sh, with a stubbed `gh` on PATH.
# Lives in its own file so each test file stays a manageable review unit.
#
# Usage: bash scripts/fetch-config-retry_test.sh
...
build_stub() {
  ...
  cat > "$work/bin/gh" <<EOF
#!/usr/bin/env bash
# Records the invoked URL for assertion, then emulates that outcome.
printf '%s\n' "\$*" >> "$work/gh-calls.log"
printf '%s' '$err' >&2
printf '%s' '$out'
exit $code
...
echo "== all fetch-config retry cases pass =="
